...
use build::ForGuard::{self, OutsideGuard, RefWithinGuard};
...
impl<'a, 'gcx, 'tcx> Builder<'a, 'gcx, 'tcx> {
                ...
                if let Some(otherwise_block) = first_candidate.otherwise_block {
                    self.cfg.terminate(
                        ...
                        TerminatorKind::FalseEdges {
                            ...
                            imaginary_targets: vec![first_candidate.next_candidate_pre_binding_block]
                        }
                    )
                } else {
    ...
    fn test_candidates<'pat, 'b, 'c>(
        ...
        let mut target_candidates: Vec<Vec<&mut Candidate>> = vec![];
        target_candidates.resize_default(target_blocks.len());
    ...
    fn calculate_fake_borrows<'b>(
        ...
        all_fake_borrows.into_iter().map(|matched_place| {
            // TODO: calculate type of Ref directly.
