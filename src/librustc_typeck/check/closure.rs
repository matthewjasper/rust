...
impl<'a, 'tcx> FnCtxt<'a, 'tcx> {
        ...
        match expected_ty.kind {
            ty::Dynamic(ref object_type, ..) => {
                let sig = object_type
                    ...
                    .next();
                let kind =
                    object_type.principal_def_id().and_then(|did| self.tcx.fn_trait_kind(did));
                (sig, kind)
            }
    ...
    fn deduce_expectations_from_obligations(
        ...
        let expected_kind = self
            ...
            .filter_map(|(tr, _)| self.tcx.fn_trait_kind(tr.def_id()))
    ...
    fn deduce_sig_from_projection(
        ...
        let is_fn = tcx.fn_trait_kind(trait_ref.def_id()).is_some();
